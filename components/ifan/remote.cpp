...
namespace esphome {
namespace ifanremote {

static const char *TAG = "ifan04";

void IFanRemote::dump_config() {
  ESP_LOGCONFIG(TAG, "");
}

void IFanRemote::loop() {
...
void IFanRemote::handle_char_(uint8_t c) {
  static int state = 0;
...
void IFanRemote::handle_command_(uint8_t type, uint8_t param) {
  if (type == 4) {
    if (param == 4)
...
void IFanRemote::output(const float state){
