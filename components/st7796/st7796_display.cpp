...
static const char *const TAG = "st7796";

void ST7796Display::setup_pins_() {
...
void ST7796Display::dump_config() {
  LOG_DISPLAY("", "st7796", this);
...
float ST7796Display::get_setup_priority() const { return setup_priority::PROCESSOR; }
void ST7796Display::command(uint8_t value) {
  this->start_command_();
  this->write_byte(value);
  this->end_command_();
}

void ST7796Display::reset_() {
  if (this->reset_pin_ != nullptr) {
...
void ST7796Display::data(uint8_t value) {
  this->start_data_();
  this->write_byte(value);
  this->end_data_();
}

void ST7796Display::send_command(uint8_t command_byte, const uint8_t *data_bytes, uint8_t num_data_bytes) {
...
uint8_t ST7796Display::read_command(uint8_t command_byte, uint8_t index) {
...
void ST7796Display::update() {
  this->do_update_();
  this->display_();
}

void ST7796Display::display_() {
...
uint16_t ST7796Display::convert_to_16bit_color_(uint8_t color_8bit) {
  int r = color_8bit >> 5;
...
uint8_t ST7796Display::convert_to_8bit_color_(uint16_t color_16bit) {
...
void ST7796Display::fill_internal_(Color color) {
...
void HOT ST7796Display::draw_absolute_pixel_internal(int x, int y, Color color) {
...
void ST7796Display::end_command_() { this->disable(); }
void ST7796Display::start_data_() {
  this->dc_pin_->digital_write(true);
  this->enable();
}
void ST7796Display::end_data_() { this->disable(); }

void ST7796Display::init_lcd_(const uint8_t *init_cmd) {
...
void ST7796Display::set_addr_window_(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h) {
  uint16_t x2 = (x1 + w - 1), y2 = (y1 + h - 1);
  this->command(ST7796_CASET);  // Column address set
  this->start_data_();
  ...
  this->write_byte(x2);
  this->end_data_();
  this->command(ST7796_PASET);  // Row address set
  this->start_data_();
  ...
  this->write_byte(y2);
  this->end_data_();
  this->command(ST7796_RAMWR);  // Write to RAM
}

void ST7796Display::invert_display_(bool invert) { this->command(invert ? ST7796_INVON : ST7796_INVOFF); }

int ST7796Display::get_width_internal() { return this->width_; }
int ST7796Display::get_height_internal() { return this->height_; }

uint32_t ST7796Display::buffer_to_transfer_(uint32_t pos, uint32_t sz) {
