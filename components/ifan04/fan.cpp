...
IFan::IFan() : EntityBase("") {}
IFan::IFan(const std::string &name) : EntityBase(name) {}

FanCall IFan::turn_on() { return this->make_call().set_state(true); }
FanCall IFan::turn_off() { return this->make_call().set_state(false); }
FanCall IFan::toggle() { return this->make_call().set_state(!this->state); }
FanCall IFan::make_call() { return FanCall(*this); }

void IFan::add_on_state_callback(std::function<void()> &&callback) { this->state_callback_.add(std::move(callback)); }
void IFan::publish_state() {
...
void IFan::dump_traits_(const char *tag, const char *prefix) {
...
uint32_t IFan::hash_base() { return 418001110UL; }
